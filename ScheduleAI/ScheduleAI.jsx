...
const s = {
  wrap: { display:'flex', flexDirection:'column', height:'100vh', maxWidth:'430px', margin:'0 auto', background:'#0B0B18', color:'#EEEEFF', fontFamily:"'DM Sans', sans-serif", overflow:'hidden', position:'relative' },
...
Si no hay tareas claras, devuelve tasks vacío y explica en reply.`;
    ...
    try {
      ...
      if (mode==='schedule') {
        if (parsed.entries?.length) {
          ...
          setMsgs(p=>[...p,{r:'ai',t:parsed.reply||'No pude extraer un horario claro. Intenta con más detalle, por ejemplo: "Matemáticas lunes y miércoles de 8 a 10am"'}]);
        }
      } else {
        if (parsed.tasks?.length) {
          ...
          await putTasks(next);
          setMsgs(p=>[...p,{r:'ai',t:(parsed.reply||'Tareas agregadas.')+'\n\n✅ Ve a "Tareas" para gestionar recordatorios de WhatsApp.'}]);
        } else {
          setMsgs(p=>[...p,{r:'ai',t:parsed.reply||'No pude extraer tareas claras. Intenta con: "Entregar trabajo de química el viernes"'}]);
        }
      }
    } catch(e) {
  ...
  const schedSuggest = ['Matemáticas lunes y miércoles 8-10am','Física martes y jueves 2-4pm','Inglés viernes 6-8pm','Historia lunes 10am-12pm'];
  const taskSuggest = ['Entregar ensayo de historia el viernes','Parcial de cálculo el martes próximo','Taller de programación en 4 días','Presentación de inglés el lunes'];
  ...
  return (
    <div style={s.wrap}>
      ...
      <div style={s.content}>
        ...
        {view === 'chatFullscreen' && (
          <div style={{display:'flex', flexDirection:'column', height:'100vh', background:'#09090B', position:'fixed', top:0, left:0, right:0, zIndex:1000}}>
            <div style={{padding:'20px 20px 15px', background:'rgba(9, 9, 11, 0.8)', backdropFilter:'blur(20px)', borderBottom:'1px solid rgba(255,255,255,.08)', display:'flex', alignItems:'center', position:'sticky', top:0, zIndex:10}}>
              <button style={{background:'none', border:'none', cursor:'pointer', display:'flex', alignItems:'center', gap:'10px', padding:0}} onClick={()=>setView('home')}>
                <div style={{width:'36px', height:'36px', borderRadius:'12px', background:'rgba(255,255,255,.05)', display:'flex', alignItems:'center', justifyContent:'center', color:'white', fontSize:'18px', transition:'all .2s'}} onMouseEnter={e=>e.currentTarget.style.background='rgba(255,255,255,.1)'} onMouseLeave={e=>e.currentTarget.style.background='rgba(255,255,255,.05)'}>‹</div>
              </button>
              <div style={{flex:1, textAlign:'center'}}>
                <div style={{fontWeight:800, fontSize:'15px', background:'linear-gradient(90deg, #E0E7FF, #C084FC)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', letterSpacing:'0.5px'}}>Asistente Schedule AI</div>
                <div style={{fontSize:'11px', color:'#34D399', fontWeight:600, display:'flex', alignItems:'center', justifyContent:'center', gap:'4px', marginTop:'2px', textTransform:'uppercase', letterSpacing:'1px'}}><div style={{width:'6px', height:'6px', background:'#34D399', borderRadius:'50%'}}></div> En Línea</div>
              </div>
              <div style={{width:'36px'}}></div> {/* Spacer para centrar título */}
            </div>
            
            <div style={{...s.cms, flex:1, padding:'20px 15px', scrollBehavior:'smooth', overflowY:'auto'}}>
              {msgs.map((m,i)=>(
                <div key={i} style={s.msgWrap}>
                  {m.r==='ai'
                    ? <div style={s.aiB}>{m.t}</div>
                    : <div style={s.usrW}><div style={s.usrB}>{m.t}</div></div>
                  }
                </div>
              ))}
              {busy && (
            ...
            <div style={{...s.cia, background:'rgba(15,16,21,.95)', backdropFilter:'blur(10px)', borderTop:'1px solid rgba(255,255,255,.05)', paddingBottom:'15px'}}>
              <div style={s.ciar}>
                <textarea
                  ref={taRef}
                  className="cita-el"
                  ...
                  onChange={e=>{
                    ...
                    if(taRef.current){taRef.current.style.height='40px'; taRef.current.style.height=Math.min(taRef.current.scrollHeight,90)+'px';}
                  }}
                  onKeyDown={e=>{if(e.key==='Enter'&&!e.shiftKey){e.preventDefault();send();}}}
                  rows={1}
                />
                <button style={s.sndbtn(!inp.trim()||busy)} onClick={send} disabled={!inp.trim()||busy}>
                  ↑
                </button>
              </div>
            </div>
