...
const upload = multer({
  ...
  fileFilter: (req, file, cb) => {
    ...
    if (allowedMimes.includes(file.mimetype) || ext === 'pdf' || ext === 'docx' || ext === 'doc') {
      ...
      cb(new Error('Ocurrió un error procesando el archivo. Verifica que sea un PDF o Word válido.'));
    }
  }
});
...
async function extractText(file) {
  ...
  const isWord = file.mimetype === 'application/vnd.openxmlformats-officedocument.wordprocessingml.document' || 
                 file.mimetype === 'application/msword' || ext === 'docx' || ext === 'doc';
  ...
  if (isWord) {
    const result = await mammoth.extractRawText({ buffer: file.buffer }).catch(() => ({ value: '' }));
    return result.value || '';
  }
  return '';
}
